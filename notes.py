...
def create(person_id, note):
    ...
    db.session.commit()  # note表中将会生成new_note

    # Serialize and return the newly created note in the response
    data = schema.dump(new_note).data  # 此时的new_note已经有note_id和timestamp了
...
def update(person_id, note_id, note):
    ...
    if update_note is not None:
        ...
        # return updated note in the response  这里有疑问，为啥不是返回update
